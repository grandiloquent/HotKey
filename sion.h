...
class String : public string {
 public:
  ...
  // num
  // 分割次数，默认-1即分割到结束，例num=1,返回开头到flag,flag到结束size=2的字符串向量
  // skip_empty 跳过空字符串，即不压入length==0的字符串
  vector<String> Split(String flag, int num = -1,
    ...
    for (auto i = 0; i < pos.size() + 1; i++) {
      if (data_set.size() == num && pos.size() > num &&
          num != -1) {  // 满足数量直接截到结束
        push_data(substr(pos[data_set.size()] + flag.size()));
        break;
      }
      if (i == 0) {  // 第一个数的位置不是0的话补上
  ...
  // newStr 新换上的字符串
  // count
  // 替换次数，默认1，大于0时替换到足够次数或找不到旧字符串为止，小于0时替换到结束
...
Socket GetSocket() {
  ...
  if (LOBYTE(inet_WsaData.wVersion) != 2 ||
      HIBYTE(inet_WsaData.wVersion) !=
          0) {  // 高位字节指明副版本、低位字节指明主版本
    WSACleanup();
    return -1;
  }
#endif
  auto tcp_socket =
      socket(AF_INET, SOCK_STREAM, 0);  // ipv4,tcp,tcp或udp该参数可为0
  return tcp_socket;
}
...
class Response {
  ...
  bool save_by_char_vec_ =
      false;  // 是否使用字符数组保存，对于文本直接用字符串保存，其它用char数组
 ...
 public:
  ...
  const String& Source() { return source_; }

  bool SaveByVec() { return this->save_by_char_vec_; }

  const String& ContentType() { return content_type_; }

  const String& Body() { return body_str_; }

  const vector<char>& BodyBin() { return body_char_vec_; }
 ...
 private:
  void HeaderAndFirstLineParse() {
    ...
    if (content_type_ != "") {  // Content-Type: text/html; charset=utf-8
      auto content_split = content_type_.Split(";", 1);
      auto type = content_split[0].Split("/", 1)[0].Trim();
      save_by_char_vec_ =
          type != "text" && type != "application";  // 解析看是文本还字节流
  ...
  void BodyStrParse() {
    if (save_by_char_vec_) {
      ...
      while (left != -1 && right != -1) {
        auto count = string(sc.begin() + 2 + left,
                            sc.begin() + right);  // 每个分块开头写的数量
        if (count == "0") {
          break;
        }  // 最后一个 0\r\n\r\n，退出
        auto count_num = stoi(count, nullptr, 16);  // 那数量是16进制
        auto chunked_start = sc.begin() + right + 2;  // 每个分块正文的开始位置
        pure_source_char.insert(pure_source_char.end(), chunked_start,
                                chunked_start + count_num);
      ...
      int right = get_next_crlf(0);
      while (left != -1 && right != -1) {
        auto count = string(rb.begin() + 2 + left,
                            rb.begin() + right);  // 每个分块开头写的数量
        if (count == "0") {
          break;
        }  // 最后一个 0\r\n\r\n，退出
        auto count_num = stoi(count, nullptr, 16);  // 那数量是16进制
        auto chunked_start = rb.begin() + right + 2;  // 每个分块正文的开始位置
        pure_str.insert(pure_str.end(), chunked_start,
                        chunked_start + count_num);
        left = get_next_crlf(count_num);  //  更新位置
        right = get_next_crlf(1);
      }
...
class Request {
 ...
 public:
  Response Send(String url) {
    ...
    check<std::invalid_argument>(m.size() == 5,
                                 "url格式不对或者是用了除http,https外的协议");
    protocol_ = m[1];
    ...
    try {
      ...
      if (protocol_ == "http") {
        string value = "CONNECT translate.google.com:80 HTTP/1.1\r\n\r\n";

        send(socket, value.c_str(), int(value.length()), 0);
        int offset = 0;
        char buf[1024] = {0};
        while (1) {
          recv(socket, buf + offset, 1, 0);
          offset++;
          if (offset >= 4) {
            if ('\n' == buf[offset - 1] && '\r' == buf[offset - 2] &&
                '\n' == buf[offset - 3] && '\r' == buf[offset - 4]) {
              break;
            }
          }
        }
 ...
 private:
  void BuildRequestString() {
    ...
    request_header_.Add("Content-Length",
                        std::to_string(request_body_.length()));
    if (cookie_.length()) {
  ...
  void Connection(Socket socket, String host) {
    in_addr sa;
    ip_ = "127.0.0.1";  //= host.HasLetter() ? GetIpByHost(host) : host;
                        /*#ifdef _WIN32
                                    check<std::invalid_argument>((InetPton(AF_INET, ip_.c_str(),
                        &sa) != -1), "地址转换错误");                     #else
                                    check<std::invalid_argument>((inet_pton(AF_INET, ip_.c_str(),
                        &sa) != -1), "地址转换错误");                     #endif*/
    sockaddr_in saddr;
    saddr.sin_family = AF_INET;
    saddr.sin_port = htons(10809);  // htons(port_);
    // saddr.sin_addr = sa;
...
#endif
  {
    ...
    auto Read = [&]() {
      ...
      check<std::runtime_error>(
          status >= 0, "网络异常,Socket错误码：" + std::to_string(status));
      return status;
    };
    Response resp;
    // 读取解析头部信息
    ...
    if (resp.save_by_char_vec_) {  // 把除头外多余的响应体部分移过去
      auto bodyPos = resp.source_.find("\r\n\r\n");
      auto startBody = buf.begin() + 4 + bodyPos;
      resp.body_char_vec_.insert(resp.body_char_vec_.end(), startBody,
                                 buf.begin() + read_count);
    }
    auto len_header =
        resp.source_.length() - resp.body_str_.length();  // 响应头长度
    // 检查是否接收完
    auto check_end = [&] {
      if (resp.save_by_char_vec_) {
        if (resp.is_chunked_) {
          auto start =
              resp.body_char_vec_.begin() + resp.body_char_vec_.size() - 7;
    ...
    while (!check_end()) {
      read_count = Read();
      if (resp.save_by_char_vec_) {
        resp.body_char_vec_.insert(resp.body_char_vec_.end(), buf.begin(),
                                   buf.begin() + read_count);
      } else {
        resp.source_ += buf.data();
      }
